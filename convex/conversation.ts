...
export const get = query({ args: { id: v.id("conversations") }, handler: async (ctx, args) => {
    ...
    if(!converasation.isGroup){
        ...
        return { ...converasation, otherMembers, otherMember: null };
    }
}})
...
export const deleteGroup = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const curentUser = await getUserByClerkId({ ctx, clerkId: identity.subject, });
...
export const leaveGroup = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const membership = await ctx.db.query("conversationMembers").withIndex("by_memberId_conversationId", (q) => q.eq("memberId", curentUser._id).eq("conversationId", args.conversationId)).unique();
